...
| A02 | Cryptographic Failures | Mitigé | Mots de passe hachés avec `bcryptjs` (salt factor 10). Token d'authentification HMAC-SHA256 signé via secret fort (≥ 64 caractères), format `base64url(userId:email:timestamp).signature`. Expiration 24 h. HTTPS obligatoire en production (nginx + certbot). |
...
| A07 | Authentication Failures | Mitigé | `ThrottlerGuard` : 5 requêtes/minute sur `/auth/*` (configurable via `THROTTLE_AUTH_LIMIT`). Politique de mot de passe : 12 caractères minimum, majuscule, minuscule, chiffre, caractère spécial. Token expiration 24 h. Pas de stockage serveur — stateless via signature HMAC. |
...
|---|---|:---:|:---:|:---:|---|---|
| R1 | Vol de token JWT via XSS (stocké en `localStorage` côté frontend) | Moyen | Élevé | 6 | `helmet` + CSP stricte, validation inputs côté backend, expiration 24 h | Rotation du `AUTH_TOKEN_SECRET` (invalide tous tokens), patch XSS côté frontend |
